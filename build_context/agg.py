...
async def run_fleet():
    ...
    while True:
        try:
            ...
            if okx:
                try:
                    # Better balance fetching for OKX
                    balance = okx.fetch_balance()
                    
                    # Try to get total USDT (Unified/Trading account)
                    total_usdt = float(balance["total"].get("USDT", 0))
                    
                    # If total is 0, try fetching sub-accounts if necessary, 
                    # but usually 'total' includes everything in unified account.
                    if total_usdt > 0:
                        equity_usd = total_usdt
                    else:
                        print(f"[WARN] OKX balance for USDT is 0. Using fallback ${equity_usd}")
                except Exception as e:
                    print(f"[ERROR] Balance fetch failed: {e}. Using fallback ${equity_usd}")
                    pass
